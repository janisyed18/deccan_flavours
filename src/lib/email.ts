...
const from = process.env.RESEND_FROM_EMAIL ?? "Deccan Flavours <orders@deccanflavours.com.au>";
...
export async function sendOrderConfirmation({
  ...
  return resend.emails.send({
    from,
    to,
    subject: `Deccan Flavours order ${orderId}`,
    html: `<h1>Order received</h1><p>Your Deccan Flavours pickup order is being prepared.</p>${renderLines(lines)}`,
  });
}
...
export async function sendReservationEmails(payload: {
  ...
  await resend.emails.send({
    from,
    to: payload.email,
    subject: "Deccan Flavours reservation request",
...
export async function sendReservationStatusEmail(payload: {
  ...
  return resend.emails.send({
    from,
    to: payload.to,
    subject: confirmed ? "Deccan Flavours reservation confirmed" : "Deccan Flavours reservation update",
