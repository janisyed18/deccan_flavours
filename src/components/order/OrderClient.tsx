    ...
    <div className="grid min-w-0 gap-8 xl:grid-cols-[220px_minmax(0,1fr)_400px]">
        ...
        <nav className="sticky top-28 grid gap-2" aria-label="Order categories">
          {menuCategories.map((group) => (
            <button
              ...
              className={`rounded border px-4 py-3 text-left text-sm font-black ${category === group.slug ? "border-forest-900 bg-forest-900 text-white" : "border-black/8 bg-white"}`}
            >
              {group.name}
            </button>
          ))}
      ...
      <section className="min-w-0">
        {orderingPause.paused ? (
          <div className="mb-5 rounded border border-forest-700/20 bg-forest-50 p-4">
            <p className="text-sm font-black uppercase tracking-[0.16em] text-forest-700">Online ordering paused</p>
            ...
            <a href={`tel:${RESTAURANT_CONFIG.phone.replace(/\s/g, "")}`} className="mt-3 inline-flex rounded bg-forest-900 px-4 py-2 text-sm font-black text-white">
              Call the restaurant
            </a>
          </div>
        ) : null}
        <div className="mb-5 flex min-w-0 gap-2 overflow-x-auto xl:hidden">
          {menuCategories.map((group) => (
            <button
              ...
              type="button"
              onClick={() => setCategory(group.slug)}
              className={`shrink-0 rounded px-4 py-2 text-sm font-black ${category === group.slug ? "bg-forest-900 text-white" : "bg-white text-charcoal"}`}
            >
              {group.name}
            </button>
          ))}
              ...
              >
                <div className="relative aspect-square overflow-hidden rounded bg-smoke">
                  ...
                  {!isAvailable ? (
                    <div className="absolute inset-0 grid place-items-center bg-charcoal/58">
                      <span className="rounded bg-white px-3 py-1 text-xs font-black text-forest-900">Sold out</span>
                    </div>
                  ) : null}
                </div>
                <div className="min-w-0">
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    ...
                    <p className="shrink-0 whitespace-nowrap font-black text-forest-700">{formatCurrency(item.price)}</p>
                  </div>
                  ...
                  <div className="mt-4 grid gap-3 border-t border-black/8 pt-3">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          isAdded ? "bg-leaf shadow-[0_0_0_4px_rgba(38,118,90,0.14)]" : "bg-forest-900 hover:bg-forest-700",
                          ...
                          {SPICE_LEVELS.map((level) => (
                            <button
                              ...
                              className={cn(
                                ...
                                draft.spiceLevel === level ? "border-forest-900 bg-forest-900 text-white" : "border-black/10 bg-white text-charcoal hover:border-forest-700/40",
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        maxLength={120}
                        className="h-10 w-full rounded border border-black/10 px-3 text-sm font-bold outline-none transition placeholder:text-charcoal/35 focus:border-forest-700 focus:ring-2 focus:ring-forest-700/15"
                      />
                    </label>
                  </div>
                </div>
      ...
      <aside className="xl:sticky xl:top-28 xl:self-start">
        <div className="rounded border border-black/8 bg-white p-5 shadow-lift">
          <div className="mb-5 flex items-center gap-2">
            <ShoppingBag aria-hidden className="h-5 w-5 text-forest-700" />
              ...
              lines.map((line) => (
                ...
                >
                  ...
                  {formatCartLineCustomization(line) ? (
                    <p className="mt-2 text-xs font-bold leading-5 text-forest-700">{formatCartLineCustomization(line)}</p>
                  ) : null}
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <p className="text-sm font-black text-forest-700">{formatCurrency(line.price * line.quantity)}</p>
                  </div>
                </div>
              ))
            )}
          ...
          {upsellSuggestions.length ? (
            <div className="mt-5 rounded border border-turmeric-700/15 bg-turmeric-100/70 p-3">
                ...
                {upsellSuggestions.map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-3 rounded bg-white p-2 text-sm">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs font-bold text-forest-700">{formatCurrency(item.price)}</p>
                    </div>
                    <button
                      type="button"
                      ...
                      className="inline-flex min-h-9 shrink-0 items-center gap-1.5 rounded bg-forest-900 px-3 text-xs font-black text-white transition hover:bg-forest-700"
                    >
                      <Plus aria-hidden className="h-3.5 w-3.5" />
                      Add
                    </button>
          ...
          <button
            ...
            onClick={checkout}
            className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded bg-forest-900 px-5 py-3 text-sm font-black text-white transition hover:bg-forest-700 disabled:cursor-not-allowed disabled:bg-charcoal/25"
          >
          ...
          {checkoutError ? <p className="mt-3 rounded border border-forest-700/20 bg-forest-50 p-3 text-sm font-bold text-forest-700">{checkoutError}</p> : null}
          <a href={RESTAURANT_CONFIG.orderingLink} target="_blank" rel="noreferrer" className="mt-4 block text-center text-sm font-black text-forest-700">
