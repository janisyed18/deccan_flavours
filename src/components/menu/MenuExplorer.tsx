          ...
          {[{ slug: "all", name: "All dishes" }, ...menuCategories].map((item) => (
            <button
              ...
              className={cn(
                ...
                category === item.slug ? "border-forest-900 bg-forest-900 text-white" : "border-black/8 bg-white text-charcoal hover:border-turmeric-300",
              )}
            >
              {item.name}
            </button>
          ...
          <div className="flex min-w-0 gap-2 overflow-x-auto pb-3 lg:hidden">
            {[{ slug: "all", name: "All" }, ...menuCategories].map((item) => (
              <button
                ...
                type="button"
                onClick={() => setCategory(item.slug)}
                className={cn("shrink-0 rounded px-4 py-2 text-sm font-black", category === item.slug ? "bg-forest-900 text-white" : "bg-white text-charcoal")}
              >
                {item.name}
              </button>
            ))}
              ...
              {filters.map((filter) => (
                <button
                  ...
                  className={cn(
                    ...
                    activeTags.includes(filter.tag) ? "border-forest-900 bg-forest-900 text-white" : "border-black/10 bg-white text-charcoal",
                  )}
                >
                  {filter.label}
                </button>
              ...
              <section key={group.slug} id={group.slug} className="scroll-mt-32">
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    ...
                    {group.slug === "plates" ? (
                      <p className="mt-2 text-sm font-bold text-forest-700">Signature plates · Dum pukht biryanis</p>
                    ) : null}
                  </div>
