  ...
  return (
    <section className="spice-texture py-16 text-white md:py-24">
      <div className="container grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div>
          <p className="mb-3 text-sm font-black uppercase tracking-[0.22em] text-turmeric-300">Click & Collect</p>
          ...
          <div className="mt-8 grid gap-3 text-sm font-bold text-white/80">
            {["Scan QR or click link", "Select items & pay via Stripe", "Track your order status live", "Pick up when ready"].map((step, index) => (
              <div key={step} className="flex items-center gap-3">
                <span className="grid h-8 w-8 place-items-center rounded bg-turmeric-300 text-xs font-black text-forest-900">{index + 1}</span>
                {step}
              </div>
            ))}
          </div>
        ...
        <div className="rounded border border-white/15 bg-white p-6 text-ink shadow-lift">
          <div className="grid gap-6 md:grid-cols-[auto_1fr] md:items-center">
            ...
            <div>
              <div className="mb-3 flex items-center gap-2 text-sm font-black uppercase tracking-[0.2em] text-forest-700">
              ...
              <Link href="/order" className="mt-6 inline-flex min-h-12 items-center gap-2 rounded bg-forest-900 px-5 py-3 text-sm font-black text-white transition hover:bg-forest-700">
