    ...
    <section className="sticky top-0 z-30 border-y border-turmeric-300/30 bg-ink text-white shadow-lg">
      <div className="container flex snap-x gap-4 overflow-x-auto py-3 text-sm font-bold md:items-center md:justify-between md:overflow-visible">
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-turmeric-100">{status.countdownLabel}</span>
        </div>
        <div className="flex shrink-0 items-center gap-2 text-white/72">
          <Clock aria-hidden className="h-4 w-4 text-turmeric-300" />
          {status.todayLabel}
        </div>
        <a href={`tel:${RESTAURANT_CONFIG.phone.replace(/\s/g, "")}`} className="flex shrink-0 items-center gap-2 text-white/72 hover:text-turmeric-300">
          <Phone aria-hidden className="h-4 w-4 text-turmeric-300" />
          {RESTAURANT_CONFIG.phone}
        </a>
        <a href={RESTAURANT_CONFIG.googleMapsLink} target="_blank" rel="noreferrer" className="flex shrink-0 items-center gap-2 text-white/72 hover:text-turmeric-300">
          <MapPin aria-hidden className="h-4 w-4 text-turmeric-300" />
          80 Station St
        </a>
      </div>
    </section>
