  ...
  return (
    <form onSubmit={submit} className="rounded border border-black/8 bg-white p-5 shadow-lift">
      <div className="mb-6 flex items-center gap-2" aria-label={`Step ${step} of 3`}>
        {[1, 2, 3].map((item) => (
          <span key={item} className={`h-2 flex-1 rounded ${item <= step ? "bg-forest-900" : "bg-smoke"}`} />
        ))}
      </div>
        ...
        <div className="grid gap-5">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <label className="block">
              ...
              {availabilityLoaded && !visibleSlots.length ? <p className="mt-2 text-xs font-bold text-forest-700">This date is fully booked for the selected party size.</p> : null}
            </label>
            <label className="block">
              <span className="mb-2 flex items-center gap-2 text-sm font-black">
      ...
      {step < 4 ? (
        <div className="mt-8 flex justify-between gap-3">
          ...
          {step < 3 ? (
            <button type="button" onClick={() => setStep((value) => value + 1)} disabled={step === 1 && !selectedTime} className="inline-flex min-h-12 items-center gap-2 rounded bg-forest-900 px-5 py-3 text-sm font-black text-white disabled:bg-charcoal/25">
              Next
              <ChevronRight aria-hidden className="h-4 w-4" />
            </button>
          ) : (
            <button type="submit" className="inline-flex min-h-12 items-center rounded bg-forest-900 px-5 py-3 text-sm font-black text-white">
              Confirm Booking
            </button>
          )}
        </div>
