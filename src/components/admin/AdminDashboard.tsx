...
export function AdminDashboard() {
  ...
  const [announcementForm, setAnnouncementForm] = useState<AnnouncementFormState>({
    ...
    message: "Fresh Deccan haleem is available this weekend while stocks last.",
    kind: "special",
    active: true,
    showOnHome: true,
    showOnMenu: true,
    ...
    <div className="grid min-w-0 gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={cn(
              ...
              active === tab.id ? "border-forest-900 bg-forest-900 text-white" : "border-black/8 bg-white text-charcoal",
            )}
          >
      ...
      <section className="min-w-0 rounded border border-black/8 bg-white p-4 shadow-lift md:p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-black/8 pb-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-turmeric-700">Admin</p>
        ...
        {message ? (
          <p className="mb-5 rounded border border-turmeric-300/40 bg-turmeric-100/65 px-4 py-3 text-sm font-bold text-forest-900" role="status">
            {message}
          </p>
        ) : null}
            ...
            <div className="grid gap-4">
              <div className="grid gap-3 lg:grid-cols-[minmax(280px,1fr)_132px_120px]">
                ...
                <button
                  type="button"
                  onClick={() => lookupOrder()}
                  className="inline-flex min-h-12 items-center justify-center gap-2 rounded bg-forest-900 px-4 text-sm font-black text-white"
                >
            ...
            <div className="min-w-0">
                ...
                {menuCategories.map((category) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedCategory === category.slug ? "bg-forest-900 text-white" : "bg-smoke text-charcoal",
                    )}
                  >
                    {category.name}
                  </button>
              ...
              <div className="mb-4 grid gap-3 rounded border border-black/8 bg-smoke p-3 md:grid-cols-[1fr_1fr_120px_auto_auto]">
                ...
                <input
                  ...
                  inputMode="numeric"
                />
                <button type="button" onClick={saveCategory} className="inline-flex h-10 items-center justify-center gap-2 rounded bg-forest-900 px-3 text-sm font-black text-white">
              ...
              <div className="grid gap-3">
                <button
                  type="button"
                  onClick={newMenuItem}
                  className="inline-flex min-h-11 w-fit items-center gap-2 rounded bg-turmeric-300 px-4 text-sm font-black text-forest-900"
                >
                  <Plus aria-hidden className="h-4 w-4" />
                  New item
                </button>
                {activeMenuItems.map((item) => (
                  <div
                    ...
                    className={cn(
                      ...
                      menuForm.id === item.id ? "border-forest-900 bg-forest-50" : "border-black/8 bg-white",
                    )}
                  >
                    ...
                    <button type="button" onClick={() => selectMenuItem(item)} className="text-right text-sm font-black text-forest-700">${item.price.toFixed(2)}</button>
                    <button
                      type="button"
            ...
            {orders.map((order) => (
              <tr key={order.id} className="border-b border-black/8">
                <td className="py-3 pr-4">
                  <button type="button" onClick={() => onSelect(order)} className="font-black text-forest-700">
                    {order.id}
                  </button>
                </td>
          ...
          <tbody>
            {reservations.map((reservation) => (
              <tr key={reservation.id} className="border-b border-black/8">
                <td className="py-3 pr-4">
                  <button type="button" onClick={() => onSelect(reservation)} className="font-black text-forest-700">
                    {reservation.id}
                  </button>
                </td>
                <td className="py-3 pr-4">
  ...
  return (
    <aside className="rounded border border-black/8 bg-smoke p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-turmeric-700">Table reservation</p>
      ...
      <div className="mt-4 grid gap-3 rounded bg-white p-3">
        ...
        <button
          ...
          className="inline-flex min-h-10 items-center justify-center gap-2 rounded bg-turmeric-300 px-3 text-xs font-black text-forest-900"
        >
          ...
          actions.map((status) => (
            <button
              ...
              className={cn(
                ...
                status === "CONFIRMED" ? "bg-leaf text-white" : "bg-forest-900 text-white",
              )}
            >
      ...
      <GuestMessageBox
        ...
        templates={[
          ["Confirm booking", `Your table at Deccan Flavours is confirmed for ${reservation.date} at ${reservation.time}.`],
          ["Late arrival", `Hi ${reservation.name}, are you still able to make your ${reservation.time} booking at Deccan Flavours?`],
          ["Cancellation notice", `Your Deccan Flavours reservation for ${reservation.date} at ${reservation.time} has been cancelled.`],
        ]}
        onMessage={onMessage}
        busy={busy}
      />
...
function OrderDetail({
  ...
  return (
    <aside className="rounded border border-black/8 bg-smoke p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-turmeric-700">Pickup order</p>
        ...
        {order.items.map((item) => (
          <div key={item.id} className="flex items-start justify-between gap-3 rounded bg-white p-3 text-sm">
            <div>
              ...
              {item.customization ? <p className="mt-1 text-xs font-bold text-forest-700">{item.customization}</p> : null}
            </div>
            <p className="font-black text-forest-700">{item.displayLineTotal}</p>
          </div>
        ))}
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        {actions.length ? (
          actions.map((status) => (
            <button
              ...
              className="inline-flex min-h-10 items-center gap-2 rounded bg-forest-900 px-3 text-xs font-black text-white disabled:bg-charcoal/25"
            >
      ...
      {order.customerEmail ? (
        <GuestMessageBox
          ...
          templates={[
            ["Pickup ready", `Your Deccan Flavours pickup order ${order.id} is ready.`],
            ["Order confirmed", `Your Deccan Flavours order ${order.id} has been received and is being prepared.`],
            ["Cancellation notice", `Your Deccan Flavours order ${order.id} has been cancelled. Please contact us if you have questions.`],
          ]}
          onMessage={onMessage}
          busy={busy}
        />
  ...
  return (
    <div className="rounded border border-black/8 bg-smoke p-4">
      <div className="mb-4 flex items-center gap-2">
        <ImagePlus aria-hidden className="h-5 w-5 text-forest-700" />
      ...
      <div className="grid gap-3">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <button type="button" onClick={onSave} disabled={busy === "menu-save"} className="inline-flex min-h-11 items-center justify-center gap-2 rounded bg-forest-900 px-4 text-sm font-black text-white disabled:bg-charcoal/25">
...
function GuestMessageBox({
  ...
  const [subject, setSubject] = useState(templates[0]?.[0] ?? "Deccan Flavours update");
  const [body, setBody] = useState(templates[0]?.[1] ?? "");

  return (
    <div className="mt-4 grid gap-3 rounded border border-black/8 bg-white p-3">
      <div className="flex items-center gap-2">
        <MessageSquare aria-hidden className="h-4 w-4 text-forest-700" />
      ...
      <button
        ...
        className="inline-flex min-h-10 items-center justify-center gap-2 rounded bg-forest-900 px-3 text-xs font-black text-white"
      >
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
          ...
          {tables.map((table) => (
            <button
              ...
              className={cn(
                ...
                table.active ? "border-forest-900 bg-white text-ink" : "border-black/10 bg-white/60 text-charcoal/45",
              )}
      ...
      <div className="rounded border border-black/8 bg-smoke p-4">
        ...
        <div className="grid gap-3">
          ...
          <button type="button" onClick={onSave} disabled={busy === "table-save"} className="inline-flex min-h-11 items-center justify-center gap-2 rounded bg-forest-900 px-4 text-sm font-black text-white">
      ...
      <div className="grid gap-3">
        {announcements.map((announcement) => (
          <button key={announcement.id} type="button" onClick={() => onForm({ ...announcement, startsAt: announcement.startsAt ?? "", endsAt: announcement.endsAt ?? "" })} className="rounded border border-black/8 p-4 text-left">
            ...
            <p className="mt-2 text-xs font-black text-forest-700">{announcement.active ? "Active" : "Inactive"}</p>
          </button>
        ))}
      </div>
      <div className="rounded border border-black/8 bg-smoke p-4">
        ...
        <div className="grid gap-3">
          ...
          <button type="button" onClick={onSave} disabled={busy === "announcement-save"} className="inline-flex min-h-11 items-center justify-center gap-2 rounded bg-forest-900 px-4 text-sm font-black text-white">
        ...
        {enquiries.map((enquiry) => (
          <button key={enquiry.id} type="button" onClick={() => onSelect(enquiry)} className="rounded border border-black/8 p-4 text-left">
            <div className="flex justify-between gap-3">
              ...
              <span className="text-xs font-black text-forest-700">{cateringStatusLabels[enquiry.status]}</span>
            </div>
        ...
        <aside className="rounded border border-black/8 bg-smoke p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(Object.keys(cateringStatusLabels) as CateringStatus[]).map((status) => (
              <button key={status} type="button" onClick={() => onUpdate(selected, status)} disabled={busy === `catering-${selected.id}`} className="rounded bg-forest-900 px-3 py-2 text-xs font-black text-white">
          ...
          <GuestMessageBox
            ...
            templates={[
              ...
              ["Deposit reminder", `Hi ${selected.name}, your Deccan Flavours catering deposit is now due to secure the date.`],
              ["Event confirmed", `Your Deccan Flavours catering event is confirmed. We look forward to serving you.`],
            ]}
            onMessage={onMessage}
            busy={busy}
          />
      ...
      <div className="flex flex-wrap gap-2">
        {ranges.map((item) => (
          <button
            ...
            className={cn("rounded px-4 py-2 text-sm font-black capitalize", range === item ? "bg-forest-900 text-white" : "bg-smoke text-charcoal")}
          >
            {item}
          </button>
        ))}
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <div key={label} className="rounded border border-black/8 bg-smoke p-4">
            ...
            <p className="mt-2 text-3xl font-black text-forest-900">{value}</p>
          </div>
        ))}
      </div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <div className="rounded border border-black/8 p-4">
        ...
        {orderingPause?.pausedUntil ? (
          <p className="mb-3 rounded bg-turmeric-100 px-3 py-2 text-sm font-bold text-forest-900">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          {[
            ...
            <button
              ...
              className="inline-flex min-h-10 items-center justify-center rounded bg-forest-900 px-3 text-sm font-black text-white disabled:bg-charcoal/25"
            >
              {label}
            </button>
          ))}
        ...
        <div className="grid gap-2">
          {Object.entries(health?.services ?? {}).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between gap-3 border-b border-black/8 py-2 text-sm">
              ...
              <span className={cn("rounded px-2 py-1 text-xs font-black", value ? "bg-leaf/10 text-leaf" : "bg-forest-500/10 text-forest-700")}>
...
function StatusBadge({ status }: { status: AdminOrderStatus }) {
  return (
    <span className="inline-flex rounded bg-forest-900 px-2.5 py-1 text-xs font-black text-white">
      {statusLabels[status]}
    </span>
  );
}
  ...
  return (
    <span
      className={cn(
        ...
        status === "REQUESTED" && "bg-turmeric-100 text-forest-900",
        status === "CANCELLED" && "bg-forest-500/10 text-forest-700",
      )}
    >
      {bookingStatusLabels[status]}
    </span>
...
function PaymentBadge({ status }: { status: AdminOrder["paymentStatus"] }) {
  return (
    <span className={cn("inline-flex items-center gap-1 rounded px-2.5 py-1 text-xs font-black", status === "PAID" ? "bg-leaf/10 text-leaf" : "bg-turmeric-100 text-forest-900")}>
