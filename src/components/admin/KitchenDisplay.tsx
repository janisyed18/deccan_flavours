...
const urgencyStyles: Record<KitchenBoardOrder["urgency"], string> = {
  ...
  warning: "border-turmeric-300 bg-turmeric-100 text-forest-900",
  late: "border-red-300 bg-red-50 text-red-800",
};

const laneStyles: Record<KitchenLane, string> = {
  new: "border-turmeric-300/50 bg-turmeric-100/50",
...
export function KitchenDisplay() {
  ...
  return (
    <section className={cn("min-h-screen bg-[#17100f] p-3 text-white md:p-5", newOrderFlash ? "ring-4 ring-turmeric-300" : "")}>
      <header className="mb-4 grid gap-3 rounded border border-white/10 bg-white/[0.06] p-4 shadow-2xl md:grid-cols-[1fr_auto] md:items-center">
        <div className="min-w-0">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <span className="rounded bg-turmeric-300 px-3 py-2 text-xs font-black uppercase tracking-[0.18em] text-forest-900">Kitchen display</span>
            {demo ? <span className="rounded border border-turmeric-300/40 px-3 py-2 text-xs font-black text-turmeric-100">Demo data</span> : null}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 md:justify-end">
          ...
          <div className="inline-flex min-h-12 items-center gap-2 rounded border border-white/10 px-3 text-sm font-black text-white/80">
            <Clock aria-hidden className="h-4 w-4 text-turmeric-200" />
          ...
          <button type="button" onClick={() => loadOrders()} className="inline-flex min-h-12 items-center gap-2 rounded bg-turmeric-300 px-4 text-sm font-black text-forest-900">
      ...
      {message ? (
        <p className="mb-4 rounded border border-turmeric-300/30 bg-turmeric-200/10 px-4 py-3 text-sm font-black text-turmeric-100" role="status">
          {message}
        </p>
      ) : null}

      {newOrderFlash ? (
        <div className="mb-4 flex items-center gap-3 rounded bg-turmeric-300 px-4 py-3 text-sm font-black text-forest-900">
    ...
    <article className="rounded border border-white/12 bg-white p-4 text-charcoal shadow-2xl">
        ...
        {order.items.map((item) => (
          <div key={item.id} className="rounded bg-smoke p-3">
            <div className="flex items-start justify-between gap-3">
              ...
              <p className="text-sm font-black text-forest-700">{item.displayLineTotal}</p>
            </div>
            {item.customization ? <p className="mt-1 text-sm font-bold text-forest-700">{item.customization}</p> : null}
          </div>
        ))}
      </div>
      ...
      {action ? (
        <button
          ...
          disabled={busy}
          className="mt-4 inline-flex min-h-14 w-full items-center justify-center gap-2 rounded bg-forest-900 px-4 text-base font-black text-white disabled:bg-charcoal/30"
        >
