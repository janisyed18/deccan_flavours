...
export const metadata: Metadata = {
  ...
  description: "Request Deccan Flavours Wentworthville catering for biryani, curries, starters, desserts, corporate events, weddings and family functions.",
  alternates: { canonical: "/catering" },
};

const cateringHighlights = [
  { icon: UtensilsCrossed, title: "Deccan Menus", text: "Biryani trays, starters, curries, naans, desserts and vegetarian packages." },
    ...
    <section className="bg-background py-12 md:py-18">
      <div className="container">
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div>
            <p className="mb-3 text-sm font-black uppercase tracking-[0.22em] text-turmeric-700">Catering</p>
            ...
            <p className="mt-5 text-lg leading-8 text-charcoal/72">
              Request catering for weddings, family functions, corporate lunches, religious events and large pickup orders from Deccan Flavours Wentworthville.
            </p>
            <div className="mt-7 grid gap-3">
              {cateringHighlights.map(({ icon: Icon, title, text }) => (
                <div key={title} className="rounded border border-black/8 bg-white p-4">
                  <div className="flex gap-3">
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded bg-turmeric-100 text-forest-900">
            ...
            <div className="mt-7 rounded border border-forest-900/10 bg-forest-900 p-5 text-white">
              <p className="text-sm font-black uppercase tracking-[0.18em] text-turmeric-300">Need urgent catering?</p>
          ...
          <ol className="mt-5 grid gap-3 md:grid-cols-4">
            {steps.map((step, index) => (
              <li key={step} className="rounded bg-smoke p-4">
                <span className="grid h-8 w-8 place-items-center rounded bg-forest-900 text-xs font-black text-white">{index + 1}</span>
