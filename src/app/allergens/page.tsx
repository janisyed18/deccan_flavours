...
export const metadata: Metadata = {
  ...
  description: "View the Deccan Flavours Wentworthville allergen matrix for menu items, including wheat, milk, egg, peanut, sesame, soy and other required allergen names.",
...
const statusStyles: Record<AllergenStatus, string> = {
  contains: "border-forest-700 bg-forest-900 text-white",
  mayContain: "border-turmeric-700/35 bg-turmeric-100 text-forest-900",
      ...
      <div className="container">
        <div className="mb-10 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
          <div className="max-w-4xl">
            <p className="mb-3 text-sm font-black uppercase tracking-[0.22em] text-turmeric-700">Allergen guide</p>
          ...
          <aside className="rounded border border-forest-900/15 bg-white p-5 shadow-sm">
            <div className="flex gap-3">
              <AlertTriangle aria-hidden className="mt-1 h-5 w-5 shrink-0 text-forest-700" />
              <div>
        ...
        <div className="mb-8 grid gap-4 lg:grid-cols-[1fr_1fr]">
          <div className="rounded border border-black/8 bg-white p-5 shadow-sm">
            <h2 className="mb-4 flex items-center gap-2 text-lg font-black text-ink">
              <Info aria-hidden className="h-5 w-5 text-turmeric-700" />
          ...
          <div className="rounded border border-black/8 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <a className="inline-flex items-center gap-1.5 text-sm font-black text-forest-700 hover:text-forest-900" href="https://www.foodstandards.gov.au/consumer/food-allergies/allergen-labelling" target="_blank" rel="noreferrer">
                FSANZ guidance <ArrowUpRight aria-hidden className="h-3.5 w-3.5" />
              </a>
              <a className="inline-flex items-center gap-1.5 text-sm font-black text-forest-700 hover:text-forest-900" href="https://www.foodauthority.nsw.gov.au/consumer/food-allergies-and-intolerances" target="_blank" rel="noreferrer">
          ...
          {groupedRows.map(({ category, rows: categoryRows }) => (
            <section key={category.slug} aria-labelledby={`allergens-${category.slug}`}>
              <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-turmeric-700">Category</p>
                  <h2 id={`allergens-${category.slug}`} className="mt-1 text-2xl font-black text-ink">
                    {category.name}
                  </h2>
                </div>
                <Link href="/contact" className="inline-flex items-center gap-2 rounded border border-forest-900/15 bg-white px-3 py-2 text-xs font-black text-forest-700 transition hover:border-forest-700">
